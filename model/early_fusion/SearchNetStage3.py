    ...
    def __init__(self, layers, depth, connections, cell_arch1, cell, dataset, num_classes, base_multiplier=40):
        ...
        for i in range(len(self.layers)):
            ...
            for j in range(self.depth):
                ...
                self.cells[i].append(cell(self.base_multiplier * multi_dict[0],
                ...
                for connection in self.connections:
                    if ([i, j] == connection[1]).all():
                        ...
                        if connection[0][0] == -1:
                            ...
                            self.fusions[i][j][str(connection[0])] = cell(
        ...
        for i in range(len(self.layers)):
            ...
            for j in range(self.depth):
                ...
                for connection in self.connections:
                    if ([i, j] == connection[1]).all():
                        if connection[0][0] == -1:
                            features[i][j] += self.fusions[i][j][str(connection[0])](pre_feature, normalized_alphas[i][j])
                        else:
