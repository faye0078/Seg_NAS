...
def image_clip(img_path, size):
    ...
    for row in range(image_row):
        for col in range(image_col):
            ...
            folder = os.path.exists("/media/dell/DATA/wy/data/GID-15-clip")
            # 判断文件夹是否存在，不存在则新建国家文件
            if not folder:  # 判断是否存在文件夹如果不存在则创建为文件夹
                os.makedirs("/media/dell/DATA/wy/data/GID-15-clip")  # makedirs 创建文件时如果路径不存在会创建这个路径
            img_filepath = '/media/dell/DATA/wy/data/GID-15-clip/' + img_name + "_" +str(row) + "_" + str(col) + ".tif"
